...
运营一项（15 分钟之后他让队伍多赢多少；不指定谁是指挥）是三样东西合成的，每一样都标了来源：
（这里用的是研究时的 45/25/30 配比和对数资历，只看形状；正式管线 build_world.py 是 40/20/40、平方根资历。）
...
def main():
    ...
    for nm in names:
        ...
        print(f'\n=== {nm}（{pos}）')
        print(' 年份 队伍            场  | 对线 操作 团战 发育 意识 协同 | 运营  ← RAPM  队伍  资历 | 个人六项均值')
        for k in keys:
            if k[1] != pos:
                continue
            y, line = k[2], k[3]
