 ...
 *
 * 第一赛段 drawn as a twelve-team triple elimination with last year's
 * 全球总决赛 sides (the region's best-regarded four in a career's first
 * year) on byes; 第二赛段 groups from six pots; the 国际赛 Swiss drawn round
 ...
 * quarter-final opponents in a drawn order; 第三赛段 reshuffled by three
 * swap pools; 全球总决赛 drawn from four pots with one side per region per
...
// ---- the 全球总决赛 2025 field the first 第一赛段 uses is what the records say
{
  ...
  console.log('全球总决赛 2025 in the records')
...
for (let i = 0; i < N; i++) {
  ...
  // ---- 第一赛段, before a ball is played: the draw is held but not written
  const myRegion = g.teams[g.myTeam].region
  {
    ...
    check(!ev.consumed && g.pendingDrawId === ev.id && !g.fixtures.some((f) => f.comp === kc.key), `${myRegion}: our 第一赛段 draw waits for us — no tie written yet`)
  ...
  for (const region of REGIONS) {
    ...
    check(kc.format === 'triple' && seeds.length === 12 && new Set(seeds).size === 12 && ev.consumed, `${region} 第一赛段: twelve seeds drawn, each once${region === myRegion ? '' : ' (held in the background)'}`)
    const real = CHAMPIONS_2025.filter((t) => kc.teams.includes(t))
    check(real.length === 4 && real.every((t) => kc.byes?.includes(t)), `${region}: first-year byes are the 全球总决赛 2025 sides (${real.map((t) => tag(g, t)).join('/')})`)
  ...
  check(!!s1?.grouped && !s1.groups && !g.fixtures.some((f) => f.comp === s1.key), '第二赛段 is a shell until its draw')
  ...
  for (const region of REGIONS) {
    ...
    check(ko.length === 30 && ko.every((f) => f.played), `${region} 第一赛段: thirty ties, all played`)
    // 胜者组决赛是 BO3 —— 第一赛段 曾是全游戏唯一一个 BO5 的胜者组决赛，跟其他
  ...
  for (const region of REGIONS) {
    ...
    check(groups.length === 2 && groups.every((x) => x.length === 6) && new Set(groups.flat()).size === 12, `${region} 第二赛段: Alpha and Omega of six, each side once`)
    const pots = s.seedPots ?? []
    check(pots.length === 6 && pots.every((p, k) => p.includes(kc.finished[k * 2]) && p.includes(kc.finished[k * 2 + 1])), `${region}: six pots by 第一赛段 placing`)
  ...
  for (const region of REGIONS) {
    ...
    check(after.length === 2 && after.every((x) => x.length === 6) && new Set(after.flat()).size === 12, `${region} 第三赛段: two groups of six, each side once`)
    ...
    check(rr.length === 30 && rr.every((f) => f.played) && !!s2c.champion, `${region}: thirty group games and a 第三赛段 champion (${tag(g, s2c.champion!)})`)
    check(Object.values(s2c.standings).every((r) => r.w + r.l <= 5) , `${region}: 第二赛段 records did not carry over`)
  }
  ...
  // ---- 全球总决赛
  {
    ...
    check(c.teams.length === 16 && groups.length === 4 && groups.every((x) => x.length === 4), '全球总决赛: sixteen in four groups of four')
    check(groups.every((x) => new Set(x.map((t) => g.teams[t].region)).size === 4), '全球总决赛: four regions in every group')
    const pots = c.seedPots ?? []
    check(pots.length === 4 && groups.every((x) => pots.every((p) => x.filter((t) => p.includes(t)).length === 1)), '全球总决赛: one side from each pot in every group')
    ...
    }), '全球总决赛: GSL openers are pot 1 v pot 4 and pot 2 v pot 3')
    ...
    check(pairs.length === 4 && pairs.every(([w, r]) => groupOf(w) !== groupOf(r)) && new Set(pairs.flat()).size === 8, '全球总决赛: each quarter-final is a group winner against another group\'s runner-up')
    check(halvesOk, '全球总决赛: a group\'s two sides sit in opposite halves')
    const ko = g.fixtures.filter((f) => f.comp === 'champions' && f.label.startsWith('KO:') && !/^[A-D]组/.test(nameOf(f)))
    check(ko.length === 14 && !!c.champion && c.finished.length === 16, `全球总决赛: fourteen playoff ties, a champion (${tag(g, c.champion!)}), sixteen placings`)
    check(gd.steps.every((st) => !st.note || st.note.includes('顺延')) && gd.log.length === 4, '全球总决赛: every forced placement carries its reason')
  }
  ...
  if (i === 0) {
    ...
    check(!!a && a === b, 'the same seed draws the same 第一赛段 bracket')
  }

  // ---- next year: this year's 全球总决赛 field takes the byes
  if (i === 0) {
    const champs = g.comps.champions.teams.slice()
    advanceDay(g); advanceDay(g)
    let guard = 0
    while (g.stage === 'offseason' && guard++ < 60) advanceDay(g)
    check(g.year === 2027 && (g.lastChampionsTeams ?? []).length === 16, `rolled into ${g.year} remembering ${g.lastChampionsTeams?.length} 全球总决赛 sides`)
    // the new year's 第一赛段 draw of our region waits for us; skip it so the byes are written
    const waiting = g.pendingDrawId
    while (g.pendingDrawId) finishDraw(g, g.draws!.find((d) => d.id === g.pendingDrawId)!, true)
    check(!!waiting, 'the new season opened on our 第一赛段 draw, waiting to be held')
    for (const region of REGIONS) {
      const kc = g.comps[`kickoff:${region}`]
      const want = champs.filter((t) => g.teams[t]?.region === region && kc.teams.includes(t))
      check(want.every((t) => kc.byes?.includes(t)), `${region} ${g.year}: last year's 全球总决赛 sides (${want.map((t) => tag(g, t)).join('/')}) have the byes`)
    }
  }
}
...
// draw to the manager must leave it ready to be chosen from.
{
  ...
  for (const tag of ['GEN', 'T1', 'BLG', 'FNC']) {
  ...
  check(opened > 0, `the manager was handed a 国际赛 pick in ${opened} of these runs`)
...
{
  const g = createNewGame(WORLD_TEAMS.find((t) => t.tag === 'GEN')!.id, 'draws', 4242)
  setupSeason(g)
  let guard = 0
...
// region was ever drawn into 第二赛段 or 第三赛段 again and the league stopped
    ...
    while (g.year === y0) {
      ...
      if (g.day === SEASON_DAYS - 2) {
        ...
        check(n('kickoff') > 0 && n('stage1') > 0 && n('stage2') > 0,
          `${y0}: 第一赛段 ${n('kickoff')}, 第二赛段 ${n('stage1')}, 第三赛段 ${n('stage2')} — all three were played`)
      }
...
// ---- 全球总决赛 groups are a draw, not a layout -----------------------------
...
// every 全球总决赛 of every career opened 美洲一号 vs 中国四号 and EMEA vs
// 太平洋.
{
  console.log('\nChampions groups')
