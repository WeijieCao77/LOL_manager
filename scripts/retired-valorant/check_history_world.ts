...
for (const year of [2024, 2025]) {
  ...
  // coaches from the opening event's participant cards; the second tier from the year's 次级联赛 tables
  ...
  check(`${year}: 美洲、欧非中东、太平洋各 8 支真实 次级联赛 队`, ['Americas', 'EMEA', 'Pacific'].every((r) => t2ByRegion[r] === 8), JSON.stringify(t2ByRegion))
  check(`${year}: 次级队也是 5～7 人`, t2.every((t) => { const n = w.players.filter((p) => p.teamId === t.id).length; return n >= 5 && n <= 7 }))
  // the 2026 world's own top 次级联赛 club is the ceiling for the year's
  const t2Max26 = Math.max(...(world26 as { teams: { tier: number; rating?: number }[] }).teams.filter((t) => t.tier === 2).map((t) => t.rating ?? 0))
  // a few points over is a strong 次级联赛 side, not a tier-one one in disguise
  ...
  // 次级联赛 tables — China's from the 2023 qualifier (2024) or the 2024
  // 次级联赛总决赛 (2025)
  {
    ...
    check(`${year}: 次级队都是当年真实的队和人，没有 2026 年的占位队`, invented.length === 0 && !t2.some((t) => t.tag === 'FK'), `${invented.length} 人对不上 ${invented.slice(0, 4).map((p) => p.ign).join(',')}`)
  }

  const m = createManager('审计', 30, 'expro')
  const club = t1.find((t) => t.tag === 'BLG') ?? t1[0]
  ...
  if (year === 2024) {
    check('2024: 第一赛段 用当年图池（微风岛屿、日落之城在）', poolOn['2024-02-16'] === 'Ascent,Bind,Breeze,Icebox,Lotus,Split,Sunset', poolOn['2024-02-16'])
    check('2024: 冠军赛用当年图池（幽邃地窟已进）', poolOn['2024-08-01'] === 'Abyss,Ascent,Bind,Haven,Icebox,Lotus,Sunset', poolOn['2024-08-01'])
  } else {
    check('2025: 第一赛段 用当年图池', poolOn['2025-01-16'] === 'Abyss,Bind,Fracture,Haven,Lotus,Pearl,Split', poolOn['2025-01-16'])
    check('2025: 第三赛段 有盐海矿镇（6 月 25 日上线）', poolOn['2025-07-18'] === 'Ascent,Bind,Corrode,Haven,Icebox,Lotus,Sunset', poolOn['2025-07-18'])
