...
console.log(`✅ 图池轮换：第一赛段 [${p0.join(' ')}]`)
console.log(`            第三赛段 [${p2.join(' ')}]`)

// --------------------------------------------------------- account unlock
const fresh = createNewGame(me.id, m.name, 7, createManager('测试', 40, 'expro'))
fresh.honours = [2027, 2028, 2029].map((year) => ({ year, title: '全球总决赛' }))
if (!earnedNow(fresh).includes('threepeat')) fail('连续三年冠军赛应解锁「三连霸」')
fresh.honours = [2026, 2028, 2029].map((year) => ({ year, title: '全球总决赛' }))
