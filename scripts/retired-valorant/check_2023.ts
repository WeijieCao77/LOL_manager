 ...
 * domestic circuit, 国际赛 Tokyo with twelve (3/4/3/2), a Last Chance
 * Qualifier per league and a Chinese qualifier for 全球总决赛, 全球总决赛 in
 * Los Angeles with sixteen; no 第一赛段 groups, no First Stand, no 第三赛段
...
const cn = t1.filter((t) => t.region === 'China').map((t) => t.tag)
check('中国十队里有当年去打国际赛的 EDG、FPX、BLG、ASE', ['BLG', 'FPX', 'ASE'].every((x) => cn.includes(x)), cn.join(','))
// the second tier is the year's own: every man a line in a 2023 次级联赛 split or the China qualifier
{
  ...
  check('次级队都是 2023 年真实打过比赛的队和人，没有 2026 年的占位队', t2.length > 0 && invented.length === 0 && !t2.some((t) => t.tag === 'FK'),
...
check('日历：季前、LOCK//IN、联赛、国际赛、LCQ、全球总决赛、休赛期，没有 First Stand 和 第三赛段', stages.join() === 'preseason,kickoff,stage1,masters2,stage2,champions,offseason', stages.join())
...
check('没有分赛区的 第一赛段', !REGIONS.some((r) => g.comps[`kickoff:${r}`]))
...
check('没有 第三赛段 联赛', !REGIONS.some((r) => g.comps[`stage2:${r}`]))
...
// Tokyo is twelve and 全球总决赛 sixteen wherever the LOCK//IN and Tokyo winners finished —
// a Tokyo of eleven never finished its Swiss, and the year stopped there
{
  ...
  for (const [from, place] of [['EMEA', 0], ['EMEA', 1], ['Pacific', 2], ['Americas', 5], ['China', 0], ['China', 4]] as const) {
    ...
    check(`东京 国际赛 十二队（LOCK//IN 冠军是 ${from} 第 ${place + 1}）`, byes.length === 4 && swiss.length === 8 && all.size === 12 && byes.includes(s.comps.kickoff.champion!), `${byes.length} 直通 + ${swiss.length} 瑞士轮，去重 ${all.size}`)
  }
  for (const from of ['EMEA', 'China'] as const) {
    const s = JSON.parse(base) as GameState
    for (const r of REGIONS) {
  ...
  function r2(from: string) { return from === 'China' ? 5 : 4 }
}
...
check('经过的赛段没有 First Stand', !seen.has('masters1') && seen.has('masters2') && seen.has('champions'))
...
check('国际赛（东京）有 12 队', news.some((n) => /MSI 季中冠军赛（东京）参赛名单/.test(n)), news.find((n) => /MSI 季中冠军赛（/.test(n))?.slice(0, 60))
check('每个赛区都有 LCQ，中国有资格赛', news.filter((n) => /LCQ名单|冠军赛中国资格赛名单/.test(n)).length === 4, String(news.filter((n) => /LCQ名单|资格赛名单/.test(n)).length))
check('全球总决赛（洛杉矶）办了', news.some((n) => /全球总决赛（洛杉矶）/.test(n)))
...
check('东京 国际赛 和洛杉矶冠军赛用当年图池（源工重镇替下森寒冬港）', poolOn['2023-06-11'] === 'Ascent,Bind,Fracture,Haven,Lotus,Pearl,Split' && poolOn['2023-08-06'] === poolOn['2023-06-11'], `${poolOn['2023-06-11']} / ${poolOn['2023-08-06']}`)
