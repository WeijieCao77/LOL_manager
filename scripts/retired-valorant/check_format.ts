 ...
 *
 *   国际赛   twelve teams; eight in a three-round Swiss, four through on two
 *             wins, four out on two losses; then a fourteen-match double
 *             elimination whose lower final and grand final are BO5
 *   全球总决赛 sixteen teams in four GSL groups of five matches each, two
 *             through per group, then the same double elimination
 *   第二赛段/2 an eight-team double elimination; 第一赛段 a four-team one
...
setupSeason(g)
let guard = 0
// the gap between our 第一赛段 ending and the 国际赛 draw: the club must
// already be able to say where it is going
// Sealed before the final: once the lower final is played, its loser is third
// and booked for 国际赛 while the champion is still undecided. Asked from
...

{
  ...
  if (c) {
    ...
    check('field = 第三赛段 top two + points top two per region', REGIONS.every((r) => field[r].every((t) => c.teams.includes(t))))
...
// the 国际赛 byes are the regional winners, the Swiss seeds the 2nd and 3rd
{
  const c = g.comps.masters1
  const winners = REGIONS.map((r) => g.comps[`kickoff:${r}`]?.finished[0]).filter(Boolean)
  const sw = fx('masters1', 'SW:')
  const swissTeams = new Set(sw.flatMap((f) => [f.teamA, f.teamB]))
  check('\nMasters I byes are the four 第一赛段 winners', !!c && winners.every((t) => c.teams.includes(t!) && !swissTeams.has(t!)))
  const seconds = REGIONS.flatMap((r) => (g.comps[`kickoff:${r}`]?.finished ?? []).slice(1, 3))
  check('First Stand Swiss field is the eight 2nd/3rd places', seconds.every((t) => swissTeams.has(t)))
}

{
  console.log(`\n=== the gap before the draw (第一赛段 China final still to play, day ${sealed?.day}) ===`)
  check('the lower-final loser is told about First Stand before the final is played',
    !!sealed?.third && sealed.third.name === 'First Stand' && sealed.third.swiss && sealed.third.day >= 66 && sealed.third.day >= sealed.day + 3,
