...
// ---- 生涯成就要在「本回合没有新东西」时也重算
{
  ...
  store.set('lolmanager:profile:' + id, JSON.stringify({
    ...before,
...
// ---- 老存档没有「接手时的阵容」，不能因此白送成就
{
  ...
  store.set('lolmanager:save:legacy', JSON.stringify(legacy))
...
{
  store.set('lolmanager:profile:X', JSON.stringify({
  ...
  store.set('lolmanager:profile:Y', '{ this is not json')
